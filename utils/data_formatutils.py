...
##### For sanity checks only: created this method to see if data leakage
def get_id(trials):
    ids = [int(trial.id.item()) for trial in trials]
    max_id = np.max(ids)
    min_id = np.min(ids)
    ids = np.asarray((ids - min_id) / (max_id - min_id + 1e-10))[:,None]
    ids = rescale_data_to_one(ids)
    return ids
