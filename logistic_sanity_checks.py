...
# make a pretty loop that runs all three situations: all data, everything but age, age only
# first make something simple though
        ...
        acc = accuracy_score(y_test[:,1], logistic_results)
        print(acc)
...
for freq, data in new_datasets.items():
    ...
    print("length of points in first patient trial", patient_trials[0].num_strips)  
    if(len(patient_trials + control_trials) != 0): 
        data, labels, stats = dfu.make_mlp_eyetrace_matrix(patient_trials, control_trials, 
                                                           params.fft_subsample)
        dataset_group_list = DatasetGroup(data, labels, stats, params)
...
# first make something simple though
        ...
        linear_results = model.predict(X_test[:,-1].reshape(-1,1))

        log_model = LogisticRegression(C=1000)
        log_model.fit(X_train[:,-1].reshape(-1,1), y_train[:,1])
        logistic_results = log_model.predict(X_test[:,-1].reshape(-1,1))
        acc = accuracy_score(y_test[:,1], logistic_results)
        print(acc)
